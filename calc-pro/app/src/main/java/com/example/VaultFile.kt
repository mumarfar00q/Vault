...
interface VaultFileDao {
    ...
    @Query("SELECT * FROM vault_files WHERE is_deleted = 0 AND (original_name LIKE '%' || :query || '%' OR tags LIKE '%' || :query || '%') ORDER BY added_at DESC")
...
abstract class VaultDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        fun unlock(context: Context, pin: String) {
            var supportFactory: net.sqlcipher.database.SupportFactory? = null
            var useSqlCipher = false
            try {
                // Safeguard against native library linkage/initialization crashes on streaming server emulators
                ...
                if (!isEmulator) {
                    ...
                    supportFactory = SupportFactory(keyBytes)
                    useSqlCipher = true
                }
            } catch (e: Throwable) {
                e.printStackTrace()
            ...
            synchronized(this) {
                ...
                if (useSqlCipher && supportFactory != null) {
                    try {
                        ...
                        // Run a test query to verify native SQLite libraries and decryption work correctly
                        val testCursor = instance.query("SELECT 1", null)
                        testCursor.close()
                    } catch (t: Throwable) {
                        t.printStackTrace()
                        try {
                ...
                if (instance == null) {
                    // Fallback to standard Room SQLite builder to prevent any application crash or linkage error
                    try {
                        instance = Room.databaseBuilder(
                            context.applicationContext,
                            VaultDatabase::class.java,
